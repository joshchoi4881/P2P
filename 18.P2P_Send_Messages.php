...
if (isset($_POST['send'])) {
		$receiverabc = htmlspecialchars($_GET['receiver']) + 0;
        ...
        if (database::query('SELECT id1 FROM users WHERE id1=:receiver', array(':receiver'=>$receiverabc))) {
        	database::query("INSERT INTO messages VALUES (:id1, :body, :sender, :receiver, :read_)", array(':id1'=>null, ':body'=>$_POST['body'], ':sender'=>$userId, ':receiver'=>$receiverabc, ':read_'=>0));
            echo "Message Sent.";
        } else {
            die('Invalid ID.');
        }
	...
	<body style="background-color: #4682B4">
		...
		<div id="wrapper">
			...
			if ($_GET['usernameLogin'] == $loggedInUserId) {
				...
				?>
				<form action="18.P2P_Send_Messages.php?usernameLogin=<?php echo $loggedInUserId; ?>&receiver=<?php echo htmlspecialchars($_GET['receiver']); ?>" method="post">
