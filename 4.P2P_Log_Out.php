...
if($_GET['usernameLogin'] != $loggedInUserId) {
	header("Location: 4.P2P_Log_Out.php?usernameLogin=$loggedInUserId");
}

if (isset($_POST['confirm'])) {
	if (isset($_POST['allDevices'])) {
		database::query('DELETE FROM logintokens WHERE userId=:userId', array(':userId'=>Login::isLoggedIn()));
		header("Location: 1.P2P_Login.php");
	} else {
		if (isset($_COOKIE['P2PID'])) {
			database::query('DELETE FROM logintokens WHERE token=:token', array(':token'=>sha1($_COOKIE['P2PID'])));
		}
		setcookie('P2PID', '1', time()-3600);
		setcookie('P2PID_', '1', time()-3600);
		header("Location: 1.P2P_Login.php");
	}
}
?>
...
#navbartwo {
	...
	<body style="background-color: #4682B4">
		...
		<div id="wrapper">
			<div class="tasks">
				<center><h1 id='tasksTitle'>Logout?</h1></center>
				<p>Are you sure you'd like to logout?</p>
				<br>
				<form action="4.P2P_Log_Out.php" method="POST">
			...
			<div class="profileLeftSideContent"></div>		
		</div>
	</body>
</html>